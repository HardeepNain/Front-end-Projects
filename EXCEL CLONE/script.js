...
cellsContainer.addEventListener("scroll", function(e) { // iska kaam hai -> ki jab bhi left right scroll kre abcd wala row saath me chle aur jab bhi up and down kre to counting wala div saath me chle
...
formulaInput.addEventListener("blur", function(e) { // jab bhi element out of focus jaayega ye event occur hoga
    ...
    if (formula) {
        ...
        //cellObject me jo formula pda wo wo formula hai jo pahle se cell pe lga hua tha // aur formula jo hai wo turant nya aaya hai kyunki wo ui se aata hai ya fir e se maanlo.
        ...
        //childrens update
        updateChildrens(cellObject.childrens); // value to formula edge case : matlab ki kisi cell me pahle value daal rkhi to manually lekin ab usme value formula ka use karke daali hai to uspe dependent cell bhi update ho jaaye : handled
    }
});

// attachClickAndBlurEventOnCell : 1.click event lagana taki jab bhi kisi cell pe click ho to us cell ka naam address me dhikhe aur ek cell se associated formula formulaInput me dhikhe
...
// solveFormula : ek formula aayega jisko solve krke return krega aur agr pahli baar sove ho rha hoga to selfCellobject bhi aayega childrens ko push krne ke liye
function solveFormula(formula, selfCellObject) {
    ...
    for (let i = 0; i < formulaComps.length; i++) {
        ...
        if ((fComp[0] >= "A" && fComp[0] <= "Z") || (fComp[0] >= "a" && fComp <= "z")) {
            ...
            // A1 || A2
            // fComp = A1
            // ab maan lo hum kaam kar rhe hai B1 pe jisme text aayega (A1+A2+C3) ko solve karke
            ...
            if (selfCellObject) { // ye condition isiliye lagayi hai taki jab hum childrens ko update kare to iske andar wala code na chale  // kyunki update ke time pe hum dobara se chilrens ko push nhi krna chahte. Ye kaam sirf first time hoga jab pahli baar formula solve hone ke liye aagega aur hume chidren pahli baar add krne honge
...
function updateChildrens(childrens) {
    for (let i = 0; i < childrens.length; i++) { // ye as a base case bhi kaam krega kyunki agr childeren honge hi nhi to unki length 0 hogi aur ye code chalega hi nhi
        ...
        //recursive call
        updateChildrens(childCellObject.childrens); // agr child ke bhi aage children honge to unko recursively call lag ke smbhal liya hai  // DFS
    }
}